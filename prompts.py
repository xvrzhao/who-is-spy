...
5. 在描述自己词语的时候，不一定要直接描述。可以描词里的某个字，例如：“张灯结彩”，可以这样说：“词中包含一个姓氏”；也可以描词的字面形式，比如：“欣欣向荣”，可以这样说：“AABC”；也可以通过关联或联想来描述，比如：“乐于助人”，可以这样说：“雷锋”；"""
...
- content：你的当众发言，所有玩家都能看到。根据 thinking 中的判断来组织——若你可能是卧底，就向平民的描述靠拢伪装自己；若你可能是平民，就与同伴的描述互相印证；不能直接说出词语本身；发言控制在100字以内；不需要做自我介绍；
...
- decision：你要投给的玩家的数字ID，只能投给尚未被淘汰的玩家；若几名玩家的可疑程度接近、把握不大，想通过下轮发言继续收集信息，可返回 0 表示弃票；
...
现在游戏结束，大家（包括已淘汰的玩家）开始交流复盘起来，以下是各玩家聊天记录（若记录为空，说明你是第一个；聊天记录按照发言顺序从前到后排列）：
