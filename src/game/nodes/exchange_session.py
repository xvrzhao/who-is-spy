...
from src.game.state import State, RawRecord
from src.game.prompts import get_rules_prompt, get_exchange_prompt
from src.game.events import emit, ExchangeSessionStart, ExchangeSessionPlayerStart, ExchangeSessionPlayerEnd, ExchangeSessionEnd
from src.game.llm import llm, ainvoke_structured
from src.game.tts import speak
...
async def exchange_session_player_node(state: State) -> State:
    ...
    if player_id == state["real_player_id"]:
        ...
        res: ExchangeStatement = await ainvoke_structured(exchange_llm, [
